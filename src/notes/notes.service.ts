...
export class NotesService {
  ...
  async getNote(agendaId: number, userId: number) {
    ...
    const note = await this.notesRepository.getNoteByAgendaAndUser(agendaId, userId);
    return note;
  }
  ...
  async saveNote(saveNoteDto: SaveNoteDto, userId: number) {
    ...
    if (!agenda) {
      throw new NotFoundException(`Agenda with ID ${saveNoteDto.agenda_id} not found`);
    }
